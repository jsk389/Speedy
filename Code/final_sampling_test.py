...
def ParallelExecutor(use_bar='tqdm', **joblib_args):
    def aprun(bar=use_bar, **tq_args):
        def tmp(op_iter):
            ...
            return joblib.Parallel(**joblib_args)(bar_func(op_iter))
        return tmp
    return aprun
...
def lnlike(params, f, p, smoo):
    ...
    if len(params) > 10:
        ...
        # Select parameters with max log-like and return those instead
        #idx = (ll == ll.max())
        #ll = ll - (np.log(1.0 / (np.sqrt(2 * np.pi) * sig)) - ((numax - params[:,4])**2 / (2.0 * sig**2)))
        #ind = np.argpartition(ll, 100)[:100]
        ind = (ll == ll.min())
        best_params = params[ind,:].squeeze()
        #return_dict['w'] = ll
        return ll[ind], best_params #, ind, best_params, np.log(1.0 / (np.sqrt(2 * np.pi) * sig)) - ((numax - params[:,4])**2 / (2.0 * sig**2)) #(ll.max(), best_params)
    else:
        mod = model(f, params)
    #plt.plot(f, p, 'k')
    #plt.plot(f, mod, 'r')
    #plt.show()
        ...
        #if not np.isfinite(ll):
        #    return 1e30 #np.inf
        return np.sum(np.log(mod) + (p / mod)) * smoo#, params[4]
...
def looped_inference(x, white, f, p, smoo):
    #print("Starting")
    #x = joblib.load('../Extract-Priors/James_complete_priors.pkl')

    samples, _ = x.sample(int(1e4))
    samples = np.exp(samples)
    #print(samples)
    #print("SAMPLING")
    samples = np.c_[samples,
                    np.ones(len(samples))*white]
    #print("LIKE")
    llike, param = lnlike(samples, f, p, smoo)
    #print("DONE!")
    return llike, param
...
def perform_inference(f, p, ff, pp, smoo, epic, plot=[], save=[]):
    ...
    # Sample in each loop, better for memory consumption!
    aprun = ParallelExecutor(n_jobs=4, backend="threading")
    #pool = Pool(3)
    #r = pool.map(looped_inference, (x, white, f, p, smoo))
    #for i in tqdm(range(n_bootstrap), total=n_bootstrap):
        #looped_inference(x, white, f, p, smoo)
    r = aprun(total=n_bootstrap)(delayed(looped_inference)
                      (x, white,
                       f, p, smoo) for i in range(n_bootstrap))
    ll, best_params = zip(*r)
    ll = np.array(ll).ravel()
    best_params = np.array(best_params)
    plt.hist(best_params[:,4], bins=30, normed=True)
    print(np.median(best_params, axis=0), np.std(best_params, axis=0))
    plt.show()
    #ll = -1.0 * ll
    #ll -= np.max(ll)
    #print("SUM: ", misc.logsumexp(ll))
    ##tot = ll - misc.logsumexp(ll)
    #print(tot, np.max(tot), np.exp(tot))
    #print(best_params)
    ...
    print("NUMAX: {} +/- {}".format(np.median(best_params, axis=0)[4],
                                    np.std(best_params, axis=0)[4]))#se[4]))
...
if __name__ == "__main__":
    ...
    f, p = np.loadtxt('../../../../../Documents/ktwo220548055_01_kasoc-psd_llc_v1.pow', unpack=True)
    fname = '/Users/james/Documents/Reggae/reggae/trunk/example_data/Interesting_spectra/4937011/kplr004937011_d15_llc_v1.pow'
    fname = '/Users/james/Documents/Reggae/reggae/trunk/example_data/Interesting_spectra/4937775/kplr004937775_d15_llc_v1.pow'
    fname = '/Users/james/Documents/Reggae/reggae/trunk/example_data/Interesting_spectra/5024414/kplr005024414_d15_llc_v1.pow'
    fname = '/Users/james/Downloads/kplr001872166_kasoc-psd_llc_v1.pow.txt'
    f, p = np.loadtxt(fname, unpack=True)
    #p = p[:len(f)//2]
    #f = f[:len(f)//2]
    #p *= 1e6
